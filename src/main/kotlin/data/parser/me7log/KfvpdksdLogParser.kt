...
object KfvpdksdLogParser {
    ...
    fun loadDirectory(directory: File, progressCallback: Me7LogParser.ProgressCallback) {
        if (directory.isDirectory) {
            scope.launch {
                try {
                    val parser = Me7LogParser()
                    val logMap = parser.parseLogDirectory(Me7LogParser.LogType.KFVPDKSD, directory, progressCallback)
                    _logs.emit(logMap)
                } catch (e: Exception) {
                    e.printStackTrace()
                }
