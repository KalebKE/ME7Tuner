...
class PlsolConstantsPanel extends JPanel {
    ...
    enum FieldTitle {
        BAROMETRIC_PRESSURE("Barometric Pressure", "mbar", true),
        INTAKE_AIR_TEMPERATURE("Intake Air Temperature", "C°", true),
        KFURL("KFURL", "%", true),
        DISPLACEMENT("Displacement", "L", true),
        RPM("RPM", "-", true);
        ...
        for (int i = 0; i < FieldTitle.values().length; i++) {
            ...
            switch (fieldTitle) {
                ...
                case KFURL:
                    ...
                    break;
                case DISPLACEMENT:
                    addDisplacement(fieldTitle, gbc, decimalFormatter);
                    break;
                case RPM:
                    addRpm(fieldTitle, gbc, decimalFormatter);
                    break;
            }
        }
    }
    ...
    private void addDisplacement(FieldTitle fieldTitle, GridBagConstraints gbc, NumberFormatter decimalFormatter) {
        final JFormattedTextField textField = new JFormattedTextField(decimalFormatter);
        textField.setValue(PlsolPreferences.getDisplacement());
        ...
        textField.getDocument().addDocumentListener(new DocumentListener() {
            ...
            public void removeUpdate(DocumentEvent e) {
                try {
                    PlsolPreferences.setDisplacement(Double.parseDouble(textField.getText()));
                    listener.onValueChanged(fieldTitle);
                } catch (NumberFormatException exception) {};
            }
    ...
    private void addRpm(FieldTitle fieldTitle, GridBagConstraints gbc, NumberFormatter decimalFormatter) {
        final JFormattedTextField textField = new JFormattedTextField(decimalFormatter);
        textField.setValue(PlsolPreferences.getRpm());
        ...
        textField.getDocument().addDocumentListener(new DocumentListener() {
            ...
            public void removeUpdate(DocumentEvent e) {
                try {
                    PlsolPreferences.setRpm(Integer.parseInt(textField.getText()));
                    listener.onValueChanged(fieldTitle);
                } catch (NumberFormatException exception) {};
            }
